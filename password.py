...
def password_generation(l):
    ...
    try:
        ...
        want_digits = bool(input("Want digits ? (Enter anything or leave empty if not): "))
        want_letters = bool(input("Want letters ? (Enter anything or leave empty if not): "))
        want_puncts = bool(input("Want punctuation ? (Enter anything or leave empty if not): "))
        if not want_digits:
            numbers = ""
        if not want_letters:
            letters = ""
...
def display_token(filename1):
    ...
    with open(filename1, "r+") as file:
        ...
        erase = bool(input("Do you want to erase the file? Enter anything or pass for NO: "))
        if erase:
            file.truncate(0)
        return
...
def compare_hash(filename2, pattern):
    with open(filename2, "r") as file:
        for l in file:
            if pattern in l:
                display_hashed = l[len(pattern) + 3:-1]
                hashed = l[len(pattern):].encode()
        print(display_hashed)
        passw = input("Entry the password corresponding to the website or application in clear:")
        passw = passw.encode()
